...
if ensure_api_connection():
    ...
    if not data_state_check(
        payload,
        error,
        empty_message="No evaluations yet. Run your first evaluation with: `python scripts/run_evaluation.py --help`",
        empty_icon="📊",
    ):
        maybe_auto_refresh()
        st.stop()
    ...
    if df.empty:
        empty_state("No evaluations yet. Run your first evaluation with: `python scripts/run_evaluation.py --help`", icon="📊")
        maybe_auto_refresh()
        st.stop()
